...
<div class="content-wrapper">
    <?php $id = $_GET['update']; $table = dbtable_w('*', 'posts','id',$id); $data = $table->fetch(); ?> 
    <form action="update_post.php?updatepost=<?php echo $data['id']; ?>" method="post" enctype="multipart/form-data">

    <div class="row add-post">
        <div class="add-new" style="margin-bottom:15px; margin-left: 10px;">
            <a class="back" href="all-posts.php?page=1"><i class="ti-arrow-left"></i>&nbsp;back</a>
        </div>
            ...
            <div class="post-card">
                ...
                <div class="post-body">
                    ...
                    <div class="form-group">
                        <input type="text" class="form-control title" id="title" name="title" placeholder="Enter title here" required value="<?php echo $data['post_title']; ?>">
                    </div>
                    <!-- Body content Start -->
                    <div class="form-group">
                        <textarea class="form-control" name="content" id="editor1" cols="30" rows="30" placeholder="Enter content here"><?php echo $data['post_content']; ?></textarea>
                    </div>
                    <!-- Excerpt Start -->
                            ...
                            <div id="excerpt" class="card-collapse collapse in">
                                <div class="card-body">
                                    <div class="form-group">
                                        ...
                                        <textarea class="form-control" name="excerpt" id="excerpt" cols="30" rows="5" placeholder="Enter excerpt here"><?php echo $data['post_excerpt']; ?></textarea>
                                    </div>
                                </div>
                            </div>
                        </div>
                            ...
                            <div id="tags" class="card-collapse collapse in">
                                <div class="card-body">
                                    <div class="form-group">
                                        ...
                                        <textarea class="form-control" name="tags" id="tags" cols="30" rows="5" placeholder="Enter tags here - separate with a comma (,)"><?php echo $data['post_meta']; ?></textarea>
                                    </div>
                                </div>
                            </div>
                        </div>
                ...
                <div class="card">
                    ...
                    <div id="menu" class="card-collapse collapsed in">
                        ...
                        <div class="card-footer">
                            <button class="btn btn-inverse-dark btn-sm trash" type="submit" name="updatetrash">Move to Trash</button>
                            ...
                            <div class="form-group">
                                ...
                                <div class="table-responsive">
                                    <table class="table">
                                        <tbody>
                                            <?php  
                                                $stmt = dbtable_order('*', 'nav_items', 'id desc'); $no = 0;
                                                $post_cat = explode(",", $data['post_cat']);
                                                foreach ($stmt as $row) { $no++;
                                                    if ($row>0) { ?>
                                                        
                                                        <tr>    
                                                            <td> <input <?php foreach ($post_cat as $key=>$value) { $cate = trim($value); if ($row['post_key']==$cate) { echo 'checked'; } } ?> name="checkbox[]" type="checkbox" value=" <?php echo $row['post_key']; ?> "/> </td>  
